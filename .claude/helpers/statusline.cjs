...
// Working directory shown in the header (added manually — ruflo omits it).
// Self-contained + guarded so it works across statusline builds and never throws.
function getWorkspaceDir() {
  var p = require('path'), os = require('os');
  var cwd = process.cwd();
  try {
    if (typeof getStdinData === 'function') {
      var d = getStdinData();
      if (d) cwd = (d.workspace && d.workspace.current_dir) || d.cwd || cwd;
    }
  } catch (e) { /* fall back to process.cwd() */ }
  if (typeof cwd !== 'string' || !cwd) cwd = process.cwd();
  var home = os.homedir();
  var dir;
  if (cwd === home) dir = '~';
  else if (home && cwd.indexOf(home + p.sep) === 0) dir = '~/' + cwd.slice(home.length + 1);
  else dir = cwd;
  if (dir.length > 24) {
    try { dir = '…/' + p.basename(p.dirname(cwd)) + '/' + p.basename(cwd); } catch (e) {}
  }
  return dir;
}
...
function generateStatusline() {
  ...
  header += '  ' + c.dim + '│' + c.reset + '  ' + c.dim + '📁 ' + getWorkspaceDir() + c.reset;
  if (git.gitBranch) {
